...
#define ARG_CASE(s, p) if (!strcmp(arg, s) && argPos + (p) < argc)
    ...
    ARG_CASE("-type", 1) {
      ...
      if (!strcmp(arg, "hardmask"))
        ...
        ABORT(
            "Invalid atlas type. Valid types are: hardmask, softmask, sdf, "
            "psdf, msdf, mtsdf");
      ++argPos;
      continue;
    }
    ARG_CASE("-format", 1) {
      ...
      if (!strcmp(arg, "png"))
        ...
        ABORT(
            "Invalid image format. Valid formats are: png, bmp, tiff, text, "
    ...
    ARG_CASE("-fontscale", 1) {
      ...
      if (!(parseDouble(fs, argv[++argPos]) && fs > 0))
        ABORT(
            "Invalid font scale argument. Use -fontscale <font scale> with a "
    ...
    ARG_CASE("-size", 1) {
      ...
      if (!(parseDouble(s, argv[++argPos]) && s > 0))
        ABORT(
            "Invalid EM size argument. Use -size <EM size> with a positive "
            "real number.");
      config.emSize = s;
      ++argPos;
      continue;
    ...
    ARG_CASE("-minsize", 1) {
      ...
      if (!(parseDouble(s, argv[++argPos]) && s > 0))
        ABORT(
            "Invalid minimum EM size argument. Use -minsize <EM size> with a "
            "positive real number.");
      minEmSize = s;
      ++argPos;
      continue;
    ...
    ARG_CASE("-emrange", 1) {
      ...
      if (!(parseDouble(r, argv[++argPos]) && r >= 0))
        ABORT(
            "Invalid range argument. Use -emrange <EM range> with a positive "
            "real number.");
      rangeMode = RANGE_EM;
      rangeValue = r;
      ++argPos;
    ...
    ARG_CASE("-pxrange", 1) {
      ...
      if (!(parseDouble(r, argv[++argPos]) && r >= 0))
        ABORT(
            "Invalid range argument. Use -pxrange <pixel range> with a "
            "positive real number.");
      rangeMode = RANGE_PIXEL;
      rangeValue = r;
      ++argPos;
    ...
    ARG_CASE("-angle", 1) {
      ...
      if (!parseAngle(at, argv[argPos + 1]))
        ABORT(
            "Invalid angle threshold. Use -angle <min angle> with a positive "
    ...
    ARG_CASE("-errorcorrection", 1) {
      ...
      if (!strcmp(argv[argPos + 1], "disabled") ||
        ...
        ABORT(
            "Unknown error correction mode. Use -errorcorrection help for more "
            "information.");
    ...
    ARG_CASE("-errordeviationratio", 1) {
      ...
      if (!(parseDouble(edr, argv[argPos + 1]) && edr > 0))
        ABORT(
            "Invalid error deviation ratio. Use -errordeviationratio <ratio> "
    ...
    ARG_CASE("-errorimproveratio", 1) {
      ...
      if (!(parseDouble(eir, argv[argPos + 1]) && eir > 0))
        ABORT(
            "Invalid error improvement ratio. Use -errorimproveratio <ratio> "
            "with a positive real number.");
    ...
    ARG_CASE("-miterlimit", 1) {
      ...
      if (!(parseDouble(m, argv[++argPos]) && m >= 0))
        ABORT(
            "Invalid miter limit argument. Use -miterlimit <limit> with a "
            "positive real number.");
      config.miterLimit = m;
      ++argPos;
      continue;
    ...
    ARG_CASE("-threads", 1) {
      ...
      if (!parseUnsigned(tc, argv[argPos + 1]) || (int)tc < 0)
        ABORT(
            "Invalid thread count. Use -threads <N> with N being a "
  ...
  if (argc == 1) {
    printf(
        ...
        ".exe"
#endif
        " -font <filename.ttf/otf> -charset <charset> <output specification> "
        "<options>\n"
  ...
  if (config.generatorAttributes.scanlinePass) {
    if (explicitErrorCorrectionMode &&
      ...
      printf(
          "Selected error correction mode not compatible with scanline mode, "
          "falling back to %s.\n",
          fallbackModeName);
    }
    config.generatorAttributes.config.errorCorrection.distanceCheckMode =
  ...
  if (imageExtension != ImageFormat::UNSPECIFIED) {
    ...
    if (mismatch)
      printf(
          "Warning: Output image file extension does not match the image's "
          "actual format (%s)!\n",
          imageFormatName);
  }
  ...
  if (config.jsonFilename) {
    ...
    */
    ...
    for (const auto &glyph : font.getGlyphs()) {
      ...
      const ab::rect_f32_t aabbAtlas(l, r, t, b);
